...
namespace ProcesosMetLife.AccesoDatos.Sistema
{
    public class Usuarios
    {
       ...
       public int ModificaLogSesion(string IdSesion, string IdUsuario, string InicioSesion, int Modo)
        ...
        //Acceso
        public DataRow Validar(string clave, string contra)
        {
            Propiedades.Usuarios usuario = new Propiedades.Usuarios();

            b.ExecuteCommandSP("SuperLogin.dbo.Usuarios_Seleccionar_Acceso");
            b.AddParameter("@clave", clave, SqlDbType.VarChar, 50);
            b.AddParameter("@contr", SuperLogin.Seguridad.Cifrado.Encriptar(contra), SqlDbType.VarChar, 50);
            return b.SelectDataRow();
        }

        public void Autorizar(string clave, string contra, IU.ManejadorSesion sesion, string app)
        {
            sesion.Inicializar();

            Propiedades.Usuarios usuario = new Propiedades.Usuarios();

            b.ExecuteCommandSP("SuperLogin.dbo.Usuarios_Seleccionar_Autorizar");
            b.AddParameter("@clave", clave, SqlDbType.VarChar, 50);
            b.AddParameter("@contra", SuperLogin.Seguridad.Cifrado.Encriptar(contra), SqlDbType.VarChar, 50);
            ...
            if (reader.HasRows)
            {
                while (reader.Read())
                {
                    usuario.IdUsuario = Funciones.Nums.TextoAEntero(reader["Id"].ToString());
                    usuario.Clave = reader["Clave"].ToString();
                    usuario.FechaRegistro = reader["FechaRegistro"].ToString();
                    usuario.Nombre = reader["Nombre"].ToString();
                    usuario.IdRol = Funciones.Nums.TextoAEntero(reader["IdRol"].ToString());
                    usuario.Activo = bool.Parse(reader["Activo"].ToString());
                    usuario.RolAcceso = reader["Acceso"].ToString();
